...
function drawCityEditOverlay() {
  ...
  if (G.pixEdit && (G.pixEdit.selNew || G.pixEdit.selDrag)) {
...
// въведеният код става цвят: приема '#abc', 'abc', '#aabbcc', 'aabbcc'
...
// след ръчно въведен цвят маркерите на палитрата застават на него
function pixSetColor(hex) {
  const e = G.pixEdit; if (!e || !hex) return;
  e.color = hex;
  const r1 = parseInt(hex.slice(1, 3), 16) / 255, g1 = parseInt(hex.slice(3, 5), 16) / 255, b1 = parseInt(hex.slice(5, 7), 16) / 255;
  const mx = Math.max(r1, g1, b1), df = mx - Math.min(r1, g1, b1);
  e.val = mx; e.sat = mx ? df / mx : 0;
  e.hue = !df ? 0 : mx === r1 ? 60 * (((g1 - b1) / df + 6) % 6) : mx === g1 ? 60 * ((b1 - r1) / df + 2) : 60 * ((r1 - g1) / df + 4);
...
// МЕСТЕНЕ на цялата рисунка с по един пиксел във всяка посока
function pixShift(dx, dy) {
  const e = G.pixEdit; if (!e) return;
  pixPushHist();
  const buf = document.createElement('canvas');
  ...
  c.drawImage(buf, dx, dy);
  if (e.sel) { e.sel = { x0: e.sel.x0 + dx, y0: e.sel.y0 + dy, x1: e.sel.x1 + dx, y1: e.sel.y1 + dy }; }
  pixSaveOverride();
}
...
function drawPixelEditor() {
  ...
  bx2 = r.x + 3 * S;
  tb('HND', bx2, 18 * S, () => { e.tool = 'hand'; }, e.tool === 'hand', 1); bx2 += 19 * S;
  tb('MIR', bx2, 18 * S, () => { e.mirror = !e.mirror; }, !!e.mirror, 1); bx2 += 19 * S;
  tb('UND', bx2, 18 * S, () => pixUndo(), false, 1); bx2 += 19 * S;
  tb('-', bx2, 10 * S, () => pixZoomStep(-1), false, 1); bx2 += 11 * S;
  tb('+', bx2, 10 * S, () => pixZoomStep(1), false, 1); bx2 += 11 * S;
  tb('RST', bx2, 18 * S, () => {
  ...
  tb('X', bx2, 11 * S, () => { G.pixEdit = null; }, false, 1);
  ...
  const isCust = e.key.slice(0, 5) === 'cust_';
  {
    ...
    tb('H+', rx, 15 * S, () => pixResizeCanvas(0, 4), false, 2); rx += 20 * S;
    // МЕСТЕНЕ на цялата рисунка (по 1 пиксел)
    tb('<', rx, 11 * S, () => pixShift(-1, 0), false, 2); rx += 13 * S;
    tb('>', rx, 11 * S, () => pixShift(1, 0), false, 2); rx += 13 * S;
    tb('^', rx, 11 * S, () => pixShift(0, -1), false, 2); rx += 13 * S;
    tb('v', rx, 11 * S, () => pixShift(0, 1), false, 2);
  }
  ...
  const prevY = pkY + huH + 4 * S, prevH = svH - huH - 4 * S;
  rcx(huX, prevY, huW, prevH, e.color);
  strokeRect(huX, prevY, huW, prevH, '#3a4456', S);
  { // КОДЪТ на цвета — чете се и се ВЪВЕЖДА с клик
    const lum = (parseInt(e.color.slice(1, 3), 16) * 0.299 + parseInt(e.color.slice(3, 5), 16) * 0.587 + parseInt(e.color.slice(5, 7), 16) * 0.114);
    ctx.font = fontBold(5.5); ctx.textAlign = 'center';
    ctx.fillStyle = lum > 140 ? '#10131c' : '#e8e4d0';
    ctx.fillText(e.color.toUpperCase(), huX + huW / 2, prevY + prevH / 2 + 2 * S);
    ctx.textAlign = 'left';
    UI.pixBtns.push({ x: huX, y: prevY, w: huW, h: prevH, act: () => {
      try {
        const t = window.prompt('Colour code (hex):', e.color.toUpperCase());
        const hx = parseHexColor(t);
        if (hx) { pixSetColor(hx); toast('Colour: ' + hx.toUpperCase(), '#7fd0a0'); }
        else if (t) toast('Not a valid code — use #aabbcc.', '#ff6b7a');
      } catch (err) {}
    } });
  }
  ...
  { // цветът ПОД КУРСОРА — за да се вижда с какво е рисувано
    let under = null;
    if (hx2 >= 0 && hy2 >= 0 && hx2 < e.spr.width && hy2 < e.spr.height &&
        G.mouse.x >= zx0 && G.mouse.x < zx0 + vw && G.mouse.y >= zy0 && G.mouse.y < zy0 + vh) {
      const d2 = e.spr.getContext('2d').getImageData(hx2, hy2, 1, 1).data;
      under = d2[3] ? ('#' + ((1 << 24) | (d2[0] << 16) | (d2[1] << 8) | d2[2]).toString(16).slice(1)).toUpperCase() : 'empty';
    }
    ctx.textAlign = 'left';
    if (under && under !== 'empty') { rcx(zx0 + 2 * S, zy0 + vh + 2 * S, 6 * S, 6 * S, under); strokeRect(zx0 + 2 * S, zy0 + vh + 2 * S, 6 * S, 6 * S, '#3a4456', 1); }
    ctx.fillStyle = '#7d8899';
    ctx.fillText(under || '', zx0 + (under && under !== 'empty' ? 10 * S : 2 * S), zy0 + vh + 8 * S);
  }
