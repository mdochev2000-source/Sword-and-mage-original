...
function drawCityEditOverlay() {
  ...
  const hint = toolH === 'move' ? 'tap a building, then a tile' : toolH === 'pencil' ? (G.pixEdit ? 'pixel editing: ' + G.pixEdit.key : 'click an object to PIXEL-EDIT its sprite') : toolH === 'place' ? (G.editPlaceKind ? 'placing: ' + G.editPlaceKind : 'PICK an element from the palette') : toolH === 'erase' ? (G.editPlaceKind ? 'erasing: ' + G.editPlaceKind + ' only' : 'PICK an element to erase') : toolH === 'create' ? 'create your own sprite — pick cells & height' : 'paint the ground';
  ...
  btn('RESET', CW / 2 - 8 * S, 44 * S, '#ff6b7a', () => {
    ...
    // недовършен щрих се записва, редакторът и CREATE се затварят чисто
    if (G.pixPaint) { G.pixPaint = false; pixSaveOverride(); }
    G.pixEdit = null; G.createSt = null; G.editPlaceKind = null; G.editSelBld = null;
    const keep = G.customDefs; // БИБЛИОТЕКАТА от създадени спрайтове оцелява при RESET
    startSurface('mirhold');
    if (keep && Object.keys(keep).length) { G.customDefs = keep; buildCustomSprites(); }
    toast('Back to the automatic layout (your sprites are kept).', '#ffd23b');
  });
  ...
  if (tool === 'place' || tool === 'erase') {
    ...
    if (G.editPlaceKind && G.editPlaceKind.slice(0, 5) === 'cust_') {
      ...
      ctx.fillText(armed ? 'SURE? DELETE FOREVER' : 'DEL SPRITE ' + delId, dx + 40 * S, dy + 7.5 * S);
...
function buildCustomSprites() {
  ...
  for (const id in defs) {
    ...
    if (Spr.custom[id] && Spr.custom[id].width === w && Spr.custom[id].height === h) continue;
...
function openPixelEditor(key) {
  ...
  G.pixEdit = { key, spr, bak, palette, color: palette[0] || '#e8e4d0', tool: 'pen', zoom: 0, panX: 0, panY: 0, hue: 0, sat: 1, val: 1 };
  Sfx.play('open');
}
...
function pixViewport() {
  const S = SCALE;
  const vw = 178 * S, vh = Math.max(60 * S, Math.min(CH - 172 * S, 340 * S));
  return { vw, vh };
}
function pixPanelRect() {
  ...
  const w = vw + 12 * S;
  const h = 46 * S + Math.ceil(e.palette.length / 8) * 12 * S + 32 * S + vh + 16 * S;
...
function pixPaintAt(mx, my) {
  ...
  if (mx < r.x || my < r.y || mx >= r.x + r.vw || my >= r.y + r.vh) return; // само в прозореца
...
function drawPixelEditor() {
  ...
  tb('HND', bx2, 20 * S, () => { e.tool = 'hand'; }, e.tool === 'hand'); bx2 += 22 * S;
  tb('-', bx2, 12 * S, () => pixZoomStep(-1), false); bx2 += 14 * S;
  tb('+', bx2, 12 * S, () => pixZoomStep(1), false); bx2 += 14 * S;
  tb('RST', bx2, 20 * S, () => {
    const c = e.spr.getContext('2d');
  ...
  if (hx2 >= 0 && hy2 >= 0 && hx2 < e.spr.width && hy2 < e.spr.height &&
    ...
    if (e.tool === 'pen') { ctx.globalAlpha = 0.55; rcx(zx0 + ox + hx2 * z, zy0 + oy2 + hy2 * z, z, z, e.color); ctx.globalAlpha = 1; }
