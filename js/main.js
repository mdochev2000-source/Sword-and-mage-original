...
function openPixelEditor(key) {
  ...
  G.pixEdit = { key, spr, bak, palette, color: palette[0] || '#e8e4d0', tool: 'pen', zoom: 0, panX: 0, panY: 0 };
  Sfx.play('open');
}
...
function pixViewport() {
  const S = SCALE;
  const vw = 178 * S, vh = Math.min(CH - 140 * S, 340 * S);
  return { vw, vh };
}
function pixPanelRect() {
  ...
  const w = vw + 12 * S;
  const h = 46 * S + Math.ceil(e.palette.length / 8) * 12 * S + vh + 16 * S;
