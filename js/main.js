...
function drawPixelEditor() {
  ...
  if (e.key.slice(0, 5) === 'cust_') { // преименуване по всяко време
    ...
    UI.pixBtns.push({ x: nx, y: ny, w: 26 * S, h: 10 * S, act: () => {
      const id = e.key.slice(5);
      try {
        const nm = (window.prompt('Name your sprite:', customName(id)) || '').trim().slice(0, 24);
        if (nm && G.customDefs[id]) { G.customDefs[id].name = nm; saveCityLayout(); toast('Named: ' + nm, '#7fd0a0'); }
      } catch (err) {}
    } });
    ctx.textAlign = 'left';
  }
