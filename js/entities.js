...
function loadProfile() {
  try { return migrateChar(JSON.parse(localStorage.getItem(charKey(G.charSlot || 0)) || 'null')); } catch (e) { return null; }
}
function wipeProfile() {
  try { localStorage.removeItem(charKey(G.charSlot || 0)); } catch (e) {}
}
function deleteChar(slot) {
  try { localStorage.removeItem(charKey(slot)); } catch (e) {}
}
