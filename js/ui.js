...
function drawShop() {
  ...
  // злато / печати / осколки — компактно едно под друго (без дупки), отместено вляво от ✕
  {
    const ixi = x0 + pw - 84 * S, ixt = x0 + pw - 70 * S;
    let iyy = y0 + 5 * S;
    ctx.font = fontBold(8);
    blit(ctx, Spr.icons.gold, ixi, iyy);
    ctx.fillStyle = '#ffd23b';
    ctx.fillText(String(p.gold), ixt, iyy + 9 * S);
    iyy += 12 * S;
    if (isMystic || G.meta.seals > 0) {
      blit(ctx, Spr.icons.seal, ixi, iyy);
      ctx.fillStyle = '#c84fff';
      ctx.fillText(String(G.meta.seals), ixt, iyy + 9 * S);
      iyy += 12 * S;
    }
    if (isMystic || (G.meta.shards || 0) > 0) {
      blit(ctx, Spr.icons.shard, ixi, iyy);
      ctx.fillStyle = '#57e6c8';
      ctx.fillText(String(G.meta.shards || 0), ixt, iyy + 9 * S);
    }
  }
