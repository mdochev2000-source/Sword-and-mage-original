...
// отместване на долния HUD (хотбар+орбове+XP) НАДОЛУ с 40px, клампнато да не излиза от екрана
// (най-ниската точка са орбовете: дъно на CH - 13*S преди отместването -> пазим 2px ръб)
function hudBottomShift() {
  return Math.max(0, Math.min(40, 13 * SCALE - 2));
}
...
function drawHUD() {
  ...
  // ЦЕНТРАЛНА КОЛБА: живот (ляво) | мана (дясно), XP пръстен-рамка, НИВОТО ОТГОРЕ
  const cx0 = CW / 2, cy0 = CH - 52 * S, orbCr = 28 * S;
  // двоен ръб: тъмен външен + фон на XP пръстена
  ctx.lineWidth = 5 * S;
  ...
  ctx.beginPath(); ctx.arc(cx0, cy0, orbCr + 4 * S, 0, Math.PI * 2); ctx.stroke();
  // половини: течност с дълбочина и ВЪЛНИСТА повърхност
  const orbHalf = (side, frac, body, deep, topCol) => {
    ctx.save();
    ctx.beginPath();
    ...
    const lvlY = cy0 + orbCr - 2 * orbCr * clamp(frac, 0, 1);
    ctx.fillStyle = body;
    ctx.fillRect(cx0 - orbCr, lvlY, orbCr * 2, 2 * orbCr);
    ctx.fillStyle = deep; // по-тъмно към дъното
    ctx.fillRect(cx0 - orbCr, Math.max(lvlY, cy0 + orbCr * 0.55), orbCr * 2, orbCr * 0.45);
    ctx.fillStyle = topCol; // вълниста повърхност (пиксел стил)
    for (let wx = -orbCr; wx < orbCr; wx += 3 * S) {
      const wy = Math.sin(wx * 0.09 / S + G.time * 2.2 + (side === 'l' ? 0 : 2)) * 1.5 * S;
      ctx.fillRect(cx0 + wx, lvlY + wy, 3 * S, 2 * S);
    }
    ctx.restore();
  };
  orbHalf('l', p.hp / p.st.maxhp, '#a81f2d', '#7c1622', '#ff5b6e');
  orbHalf('r', p.mp / p.st.maxmp, '#1f4d99', '#173a7a', '#6aa8ff');
  // стъклен отблясък горе-вляво
  ctx.save();
  ctx.beginPath(); ctx.arc(cx0, cy0, orbCr, 0, Math.PI * 2); ctx.clip();
  ctx.globalAlpha = 0.10;
  ctx.fillStyle = '#ffffff';
  ctx.beginPath(); ctx.arc(cx0 - orbCr * 0.28, cy0 - orbCr * 0.34, orbCr * 0.55, 0, Math.PI * 2); ctx.fill();
  ctx.globalAlpha = 1;
  ctx.restore();
  ...
  // XP прогресът тръгва от ДЪНОТО (за да не се крие под нивото отгоре)
  ...
  // САМО количеството (без „/макс"), със сянка за четимост
  ctx.textAlign = 'center';
  ctx.font = fontBold(9);
  const orbNum = (v, x) => {
    ctx.fillStyle = 'rgba(5,7,12,0.85)';
    ctx.fillText(v, x + S, cy0 + 3 * S + S);
    ctx.fillStyle = '#ffffff';
    ctx.fillText(v, x, cy0 + 3 * S);
  };
  orbNum(String(Math.ceil(p.hp)), cx0 - orbCr * 0.5);
  orbNum(String(Math.ceil(p.mp)), cx0 + orbCr * 0.5);
  // НИВОТО — ОТГОРЕ на колбата (златна табелка)
  {
    const lt = 'LV ' + p.lvl;
    ctx.font = fontBold(6);
    const ltw = ctx.measureText(lt).width;
    const bw2 = Math.max(24 * S, ltw + 8 * S);
    rcx(cx0 - bw2 / 2, cy0 - orbCr - 10 * S, bw2, 11 * S, '#0e121c');
    strokeRect(cx0 - bw2 / 2, cy0 - orbCr - 10 * S, bw2, 11 * S, '#c8a832', S);
    ctx.fillStyle = '#ffd23b';
    ctx.fillText(lt, cx0, cy0 - orbCr - 2 * S);
  }
  ...
  // подредба САМО ЛЯВО и ДЯСНО от колбата, на колони (нищо отгоре/отдолу)
  const colInL = [ // вътрешна лява колона: оръжие + скок + Камъка на живота (под скока)
    ...
    { key: 'SS', soulstoneSlot: true },
  ];
  const colOutL = [potionSlot(0), potionSlot(1)]; // външна лява: двете отвари
  const colInR = [ // вътрешна дясна: трите активни магии
    spellSlot(0, 'RMB', true),
    spellSlot(1, '3', G.meta.magic3),
    spellSlot(2, '4', G.meta.magic4),
  ];
  const colOutR = [{ key: 'P1', passive: 0 }, { key: 'P2', passive: 1 }]; // външна дясна: двете аури
  const sw = 24 * S; // клетките са залепени
  const pas = p.activePassives || [null, null];
  const drawSlot = (s, sx, sy) => {
    panel(sx, sy, sw, sw);
    if (s.passive !== undefined) {
      // пасивен слот (аура): руна + пулс + резервирана мана; тап отваря Книгата
      const pid = pas[s.passive];
      if (pid && SPELLS[pid]) {
        const sp2 = SPELLS[pid];
        const cx0 = sx + sw / 2, cy0 = sy + sw / 2;
        ctx.fillStyle = sp2.col;
        ctx.beginPath();
        ctx.moveTo(cx0, cy0 - 6 * S); ctx.lineTo(cx0 + 5 * S, cy0); ctx.lineTo(cx0, cy0 + 6 * S); ctx.lineTo(cx0 - 5 * S, cy0);
        ctx.closePath(); ctx.fill();
        ctx.globalAlpha = 0.3 + 0.2 * Math.sin(G.time * 3 + s.passive);
        strokeRect(sx + S, sy + S, sw - 2 * S, sw - 2 * S, sp2.col, S);
        ctx.globalAlpha = 1;
        ctx.font = fontPx(4.5); ctx.textAlign = 'right'; ctx.fillStyle = '#7fb0ff';
        ctx.fillText(String(50 * ((p.spellLvl && p.spellLvl[pid]) || 1)), sx + sw - 2 * S, sy + sw - 2 * S);
        ctx.textAlign = 'center';
      } else {
        rcx(sx + 2 * S, sy + 2 * S, sw - 4 * S, sw - 4 * S, 'rgba(12,15,24,0.6)');
        ctx.fillStyle = '#2c3444';
        ctx.fillRect(sx + sw / 2 - S, sy + sw / 2 - S, 2 * S, 2 * S);
      }
      ctx.font = fontPx(5); ctx.fillStyle = '#7d8899';
      ctx.fillText(s.key, sx + sw / 2, sy + 6 * S);
      UI.hotRects.push({ x: sx, y: sy, w: sw, h: sw, act: ((idx) => () => { openSpellbook(); G.sbSel = 3 + idx; })(s.passive) });
      return;
    }
    if (s.soulstoneSlot) {
      // Камъкът на живота: вижда се, че е екипиран (розова пулсираща рамка); празен слот, когато го няма
      if (p.equip.soulstone) {
        blit(ctx, Spr.icons.soulstone, sx + sw / 2 - 6 * S, sy + sw / 2 - 7 * S);
        ctx.globalAlpha = 0.35 + 0.2 * Math.sin(G.time * 3);
        strokeRect(sx + S, sy + S, sw - 2 * S, sw - 2 * S, '#ff8aa0', S);
        ctx.globalAlpha = 1;
      } else {
        rcx(sx + 2 * S, sy + 2 * S, sw - 4 * S, sw - 4 * S, 'rgba(12,15,24,0.6)');
        ctx.fillStyle = '#2c3444';
      ...
      ctx.font = fontPx(5); ctx.fillStyle = '#7d8899';
      ctx.fillText(s.key, sx + sw / 2, sy + 6 * S);
      UI.hotRects.push({ x: sx, y: sy, w: sw, h: sw, act: () => { G.state = 'inventory'; document.body.classList.add('menu'); } });
      return;
    }
    if (s.locked) {
      // заключен слот за магия: катинарче (Майсторът го отваря срещу Печат)
      rcx(sx + 2 * S, sy + 2 * S, sw - 4 * S, sw - 4 * S, 'rgba(8,10,16,0.55)');
      const cx0 = sx + sw / 2, cy0 = sy + sw / 2;
      ...
      rcx(sx + 2 * S, sy + 2 * S, sw - 4 * S, sw - 4 * S, 'rgba(30,20,50,0.5)');
      strokeRect(sx + 5 * S, sy + 5 * S, sw - 10 * S, sw - 10 * S, '#6a4f9e', S);
      ...
      drawPotionGlyph(sx + sw / 2, sy + sw / 2 + S, s.potion.col);
    }
    else if (s.emptyPotion) {
      // празен слот за отвара (валиден билд: 1 отвара + бъф)
      rcx(sx + 2 * S, sy + 2 * S, sw - 4 * S, sw - 4 * S, 'rgba(20,25,38,0.5)');
      ...
      ctx.fillText('»', sx + sw / 2, sy + sw / 2 + 3 * S);
      // пипове за зарядите
      for (let k = 0; k < p.st.dashMax; k++) {
        ctx.fillStyle = k < (p.dashCharges || 0) ? '#ffd23b' : '#454e63';
        ctx.fillRect(sx + 4 * S + k * 5 * S, sy + sw - 5 * S, 3 * S, 2 * S);
      }
    }
    else blit(ctx, Spr.icons[s.icon], sx + sw / 2 - 6 * S, sy + sw / 2 - 7 * S);
    if (s.cd > 0) rcx(sx, sy + sw * (1 - clamp(s.cd, 0, 1)), sw, sw * clamp(s.cd, 0, 1), 'rgba(8,10,16,0.65)');
    ctx.font = fontPx(5);
    ...
    if (s.info !== null && s.info !== undefined) {
      ...
      ctx.fillText(s.info, sx + sw - 5 * S, sy + sw - 3 * S);
    }
    if (s.act) UI.hotRects.push({ x: sx, y: sy, w: sw, h: sw, act: s.act });
  };
  // колоните са вертикално центрирани спрямо колбата
  const colInLx = cx0 - orbCr - 6 * S - sw, colOutLx = colInLx - sw;
  const colInRx = cx0 + orbCr + 6 * S, colOutRx = colInRx + sw;
  // ШАХМАТНО и от двете страни: вътрешните колони (3 слота) започват от -36, външните (2) от -24
  colInL.forEach((s, i) => drawSlot(s, colInLx, cy0 - 36 * S + i * sw));   // оръжие + скок + Камъка (до колбата)
  colOutL.forEach((s, i) => drawSlot(s, colOutLx, cy0 - sw + i * sw));     // двете отвари (навън, изместени)
  colInR.forEach((s, i) => drawSlot(s, colInRx, cy0 - 36 * S + i * sw));   // трите магии (до колбата)
  colOutR.forEach((s, i) => drawSlot(s, colOutRx, cy0 - sw + i * sw));     // двете аури (навън, изместени)
  ...
  // подсказка за взаимодействие: на клавиатура текст "E — ..."; на тъч е КРЪГЛИЯТ местим бутон (в контролите)
  if (G.interactHint && !G.isTouch) {
    ctx.textAlign = 'center';
    const txt = G.interactHint.txt;
    ctx.font = fontBold(7);
    const tw = ctx.measureText(txt).width;
    const hx = CW / 2, hy = CH - 98 * S; // точно над кръглия HUD
...
function invWindowGeom() {
  ...
  // СТАРАТА котва (отпреди кръглия HUD): прозорците застават ниско и се показват НАД (върху)
  // хотбара — той се рисува под тях. Важи и за компютър, и за мобилни (формулата е по SCALE).
  const xpY = CH - 42 * SCALE + hudBottomShift();
  const ph = Math.min(xpY - 5 * SCALE - 8, Math.max(phA * SCALE, phA * PS - 16 * SCALE));
  const x0 = (CW - pw) / 2;
  // долният ръб е съвсем малко над жълтата XP лента
  const y0 = Math.max(8, xpY - 5 * SCALE - ph);
  return { x0, y0, pw, ph, cols, invRows, gridW, BORDER };
}
function drawInventory() {
  const p = G.player;
  UI.invRects = []; UI.equipRects = []; UI.invDropRect = null; UI.invEquipRect = null; UI.invUnequipRect = null;
  const gm = invWindowGeom();
  const x0 = gm.x0, y0 = gm.y0, pw = gm.pw, ph = gm.ph, cols = gm.cols, invRows = gm.invRows, gridW = gm.gridW, BORDER = gm.BORDER;
  panel(x0, y0, pw, ph);
  ...
  // мини-статистики — ЕДНА плътна колонка до екипировката (етикет отгоре, число отдолу, без празнини)
  {
    const st = p.st;
    const statCol = [
      ['DMG', Math.round(st.dmg)],
      ['CRIT', Math.round(st.crit) + '%'],
      ['ATK/S', (1 / st.atkCd).toFixed(2)],
      ['ARMOR', st.armor],
      ['LIFE', st.maxhp],
      ['STEAL', st.vamp + '%'],
  ...
  // бутони НАЙ-ДОЛУ, един до друг, точно под колоната с предмети (къси думи, без символи/точки)
  {
    const bw3 = 44 * S, bh3 = 13 * S, gap3 = 3 * S; // 3x44 + 2x3 = 138 art = ширината на мрежата
    const bx3 = gx0, by3 = y0 + ph - bh3 - 5 * S;
    ctx.font = fontBold(6);
    ctx.textAlign = 'center';
    const btnDefs = [
      { t: 'STATS', col: '#8ab0ff', key: 'openStats' },
      { t: 'SPELLS', col: '#8ab0ff', key: 'openBook' },
      { t: 'SKILLS', col: (p.skillPoints || 0) > 0 ? '#ffd23b' : '#7fd0a0', key: 'openTree' }, // жълто = има точки
    ];
  ...
  // тултип и сравнение — в празното пространство по средата (по-добра видимост)
  UI.hoverItem = null;
  ...
  const zx = x0 + 70 * S;                   // ляв ръб на зоната — плътно до колоната със статистики (тя свършва ~x0+63)
  ...
  if (G.equipSel && p.equip[G.equipSel]) {
    ...
    let cur = null; // срещу какво сравняваме (за пръстен — слотът, който би бил сменен)
    if (it.slot === 'ring') cur = (p.equip.ring && p.equip.ring2) ? p.equip.ring : (p.equip.ring || p.equip.ring2);
  ...
  if (G.isTouch && G.invSel !== null && G.invSel !== undefined && p.inv[G.invSel]) {
    ...
    const bw = 53 * S, bh = 14 * S, by = y0 + ph - bh - 5 * S; // НАЙ-ДОЛУ, под сравнението по средата
    const canEq = selIt.slot !== 'spell';
    const eqR = { x: midX - bw - 2 * S, y: by, w: bw, h: bh };
    ...
    const drR = { x: midX + 2 * S, y: by, w: bw, h: bh };
...
function drawShop() {
  ...
  UI.shopInvRects = [];
  UI.enchPotRects = [];
  UI.shopListRect = null; // скрол-зоната на списъка на търговеца
  UI.enchAffixRects = []; UI.shopActRects = [];
  // 1:1 С ПРОЗОРЕЦА НА ИНВЕНТАРА: същите размери и позиция
  const gm = invWindowGeom();
  ...
  // валутите — на ЕДИН РЕД: среброто си стои, вляво от него осколките, после печатите
  {
    ...
    if (isMystic || (G.meta.shards || 0) > 0) {
      ix -= 42 * S;
      blit(ctx, Spr.icons.shard, ix, iy);
      ctx.fillStyle = '#57e6c8';
      ctx.fillText(String(G.meta.shards || 0), ix + 14 * S, iy + 9 * S);
    }
    if (isMystic || G.meta.seals > 0) {
      ix -= 42 * S;
      blit(ctx, Spr.icons.seal, ix, iy);
      ctx.fillStyle = '#c84fff';
  ...
  if (canExch) {
    const label = G.shopSilver ? 'BACK TO WARES' : 'WEIGH HACKSILVER';
    ctx.font = fontBold(5); // надписът трябва да СЕ СЪБИРА в рамката
    const bw = Math.max(70 * S, ctx.measureText(label).width + 24 * S);
    const bx3 = x0 + 10 * S, by3 = y0 + 26 * S;
    panel(bx3, by3, bw, 12 * S);
    strokeRect(bx3, by3, bw, 12 * S, G.shopSilver ? '#7fd0a0' : '#3a4456', S);
    blit(ctx, Spr.icons.sv_bag, bx3 + 3 * S, by3 + 1 * S);
    ctx.font = fontBold(5); ctx.fillStyle = G.shopSilver ? '#7fd0a0' : '#a8b2c4';
    ctx.textAlign = 'left';
    ctx.fillText(label, bx3 + 16 * S, by3 + 8 * S);
  ...
  let ry = listTop - G.shopScroll;
  const rowVisible = () => ry + rowH > listTop && ry < listBottom;
  if (isMystic) {
    // отключвания срещу Печати
    for (const of2 of offers) {
      if (rowVisible()) {
        const hov = G.mouse.x >= listX && G.mouse.x < listX + rowW && G.mouse.y >= ry && G.mouse.y < ry + rowH;
        rcx(listX, ry, rowW, rowH, hov ? 'rgba(80,50,110,0.5)' : 'rgba(30,20,50,0.6)');
        if (hov) strokeRect(listX, ry, rowW, rowH, '#6a4f9e', 1);
        blit(ctx, Spr.icons.seal, listX + 3 * S, ry + 3 * S);
        ctx.font = fontPx(6.5); ctx.fillStyle = '#e8e4d0';
        ctx.fillText(of2.t, listX + 18 * S, ry + 8 * S);
        ctx.fillStyle = '#7d8899'; ctx.font = fontPx(5.5);
        ctx.fillText(of2.d, listX + 18 * S, ry + 15 * S);
        ctx.textAlign = 'right'; ctx.font = fontBold(6.5);
        ctx.fillStyle = of2.cost === 0 ? '#7fd0a0' : (G.meta.seals >= of2.cost ? '#c84fff' : '#ff6b7a');
        ctx.fillText(of2.cost === 0 ? 'free' : of2.cost + ' seal' + (of2.cost > 1 ? 's' : ''), listX + rowW - 4 * S, ry + 11 * S);
        ctx.textAlign = 'left';
        UI.shopRects.push({ x: listX, y: ry, w: rowW, h: rowH, entry: { unlock: of2 } });
      }
      ry += rowStride;
    }
    if (!offers.length) {
      if (rowVisible()) { ctx.fillStyle = '#7d8899'; ctx.font = fontPx(6); ctx.fillText('Everything structural is unlocked.', listX, ry + 10 * S); }
      ry += rowStride;
    }
    if (rowVisible()) { ctx.font = fontBold(6); ctx.fillStyle = '#57e6c8'; ctx.fillText('POTIONS (upgrade, shards)', listX, ry + 12 * S); }
    ry += rowStride;
    for (const key of ownedP) {
      if (rowVisible()) {
        const pd = POTIONS[key], up = (p.potionUp && p.potionUp[key]) || 0, maxed = up >= 3, cost = potionEnchantCost(up);
        const hov = G.mouse.x >= listX && G.mouse.x < listX + rowW && G.mouse.y >= ry && G.mouse.y < ry + rowH;
        rcx(listX, ry, rowW, rowH, (hov && !maxed) ? 'rgba(40,90,80,0.5)' : 'rgba(20,35,32,0.6)');
        if (hov && !maxed) strokeRect(listX, ry, rowW, rowH, '#57e6c8', 1);
        drawPotionGlyph(listX + 7 * S, ry + 9 * S, pd.col);
        ctx.textAlign = 'left'; ctx.font = fontPx(6.5); ctx.fillStyle = pd.col;
        ctx.fillText(pd.n + ' ' + '★'.repeat(up) + '☆'.repeat(3 - up), listX + 18 * S, ry + 8 * S);
        ctx.font = fontPx(5.5); ctx.fillStyle = '#7d8899';
        ctx.fillText(maxed ? 'fully upgraded' : '−12% cd · +12% power', listX + 18 * S, ry + 15 * S);
        if (!maxed) {
          ctx.textAlign = 'right'; ctx.font = fontBold(6.5);
          ctx.fillStyle = (G.meta.shards || 0) >= cost ? '#57e6c8' : '#ff6b7a';
          ctx.fillText(cost + ' shd', listX + rowW - 4 * S, ry + 11 * S);
          ctx.textAlign = 'left';
          UI.enchPotRects.push({ x: listX, y: ry, w: rowW, h: rowH, key });
        }
      }
      ry += rowStride;
    }
  } else {
    // стока: тап МАРКИРА (после Buy с потвърждение)
    for (const entry of stock) {
      if (rowVisible()) {
        const sel = G.selStock === entry;
        const hov = G.mouse.x >= listX && G.mouse.x < listX + rowW && G.mouse.y >= ry && G.mouse.y < ry + rowH;
        rcx(listX, ry, rowW, rowH, sel ? 'rgba(70,92,120,0.8)' : (hov ? 'rgba(60,70,95,0.5)' : 'rgba(20,25,38,0.7)'));
        if (sel || hov) strokeRect(listX, ry, rowW, rowH, sel ? '#ffd23b' : '#5a677f', 1);
        ctx.font = fontPx(6.5);
        if (entry.potion) {
          const pd = POTIONS[entry.potion];
          drawPotionGlyph(listX + 7 * S, ry + 9 * S, pd.col);
          ctx.fillStyle = pd.col;
          ctx.fillText(pd.n + ' (unlock)', listX + 18 * S, ry + 8 * S);
          ctx.fillStyle = '#7d8899'; ctx.font = fontPx(5.5);
          ctx.fillText(pd.d.slice(0, 32), listX + 18 * S, ry + 15 * S);
        } else {
          blit(ctx, Spr.icons[entry.item.icon], listX + 3 * S, ry + 3 * S);
          ctx.fillStyle = Items.rarityCol(entry.item);
          ctx.fillText(entry.item.name, listX + 18 * S, ry + 8 * S);
          ctx.fillStyle = '#7d8899'; ctx.font = fontPx(5.5);
          const st = entry.item.dmg ? entry.item.dmg + ' damage' : entry.item.armor ? entry.item.armor + ' armor' : RARITY[entry.item.rarity].n;
          ctx.fillText(st + (entry.item.affixes.length ? ' · +' + entry.item.affixes.length + ' properties' : ''), listX + 18 * S, ry + 15 * S);
        }
        ctx.textAlign = 'right'; ctx.font = fontBold(6.5);
    ...
    if (lvl < 5 && !isPeddler) {
      if (rowVisible()) {
        ...
        rcx(listX, ry, rowW, rowH, hovU ? 'rgba(60,90,60,0.5)' : 'rgba(25,40,28,0.7)');
        if (hovU) strokeRect(listX, ry, rowW, rowH, '#7fd0a0', 1);
        ctx.font = fontBold(6.5); ctx.fillStyle = '#7fd0a0';
        ctx.fillText('⌂ Upgrade: ' + STALL_NAMES[lvl - 1] + ' → ' + STALL_NAMES[lvl], listX + 4 * S, ry + 8 * S);
        ctx.font = fontPx(5.5); ctx.fillStyle = '#7d8899';
  ...
  if (maxScroll > 0) {
    ...
    rcx(listX + rowW + 2 * S, thumbY, 2 * S, thumbH, '#6a7690');
  }

  // === ДЯСНА КОЛОНА: филтри + мрежа с моите предмети — 1:1 като в инвентара (долепена вдясно) ===
  const gx0 = x0 + pw - (gridW + BORDER) * S, gy0 = y0 + 40 * S;
  const cw2 = 26 * S, gp = 2 * S;
  const invFilt = G.invFilter || 'weapon';
  const gridItems = [];
  if (isMystic) {
    for (const slot of ['weapon', 'armor', 'ring', 'ring2', 'amulet']) { const it = p.equip[slot]; if (it && it.affixes && it.affixes.length && itemFilterCat(it) === invFilt) gridItems.push(it); }
    for (const it of p.inv) if (it.slot !== 'spell' && it.affixes && it.affixes.length && itemFilterCat(it) === invFilt) gridItems.push(it);
  } else {
  ...
  // 3-те филтъра над мрежата — при ВСИЧКИ продавачи (като в инвентара)
  UI.shopFilterRects = [];
  {
    const fw = 44 * S, fh = 12 * S, fy = y0 + 24 * S;
    ctx.textAlign = 'center'; ctx.font = fontBold(5.5);
    INV_FILTERS.forEach((f, fi) => {
      const fx = gx0 + fi * (fw + 3 * S);
  ...
  // мрежа с моите предмети — празните клетки също се рисуват (като в инвентара)
  const totalCells = Math.max(gm.invRows * cols, Math.ceil(gridItems.length / cols) * cols);
  for (let ci = 0; ci < totalCells; ci++) {
    const rx = gx0 + (ci % cols) * (cw2 + gp), ryy = gy0 + ((ci / cols) | 0) * (cw2 + gp);
    panel(rx, ryy, cw2, cw2);
    const it = gridItems[ci];
    if (it) {
      ...
      if (it.lvl) {
        ctx.font = fontPx(4.5); ctx.textAlign = 'right'; ctx.fillStyle = '#a8b2c4';
        ctx.fillText(it.lvl, rx + cw2 - 2 * S, ryy + cw2 - 2 * S);
        ctx.textAlign = 'left';
      }
      UI.shopInvRects.push({ x: rx, y: ryy, w: cw2, h: cw2, item: it });
    }
  }
  // === СРЕДА: инфо/сравнение в ДВЕ РАВНИ ПОЛОВИНИ — 1:1 като в инвентара ===
  const zx = x0 + 146 * S;              // след списъка на търговеца
  const zw = gx0 - 4 * S - zx;
  const zTop = y0 + 26 * S, zBot = y0 + ph - 26 * S;
  const zMid = (zTop + zBot) / 2, topH = zMid - 2 * S - zTop, botH = zBot - (zMid + 2 * S);
  const midX = zx + zw / 2;
  const btnY = y0 + ph - 22 * S, btnH = 18 * S; // ред за действия — най-долу в средата
  // панел, запълващ половината (като в инвентара)
  const half = (it, y, hh, header, extra) => {
    const hArt = 26 + (header ? 9 : 0) + Items.statLines(it).length * 9 + (extra ? extra.length * 9 : 0);
    const k = Math.min(zw / 96, hh / hArt);
    tooltipPanel(it, zx, y, header, extra || null, k, zw, hh);
  };
  const actBtn = (label, bx, w, act, col) => {
    const r = { x: bx, y: btnY, w, h: btnH, act };
    panel(r.x, r.y, r.w, r.h);
    strokeRect(r.x, r.y, r.w, r.h, col, S);
    ctx.font = fontBold(7); ctx.textAlign = 'center'; ctx.fillStyle = col;
    ctx.fillText(label, r.x + w / 2, btnY + 12 * S);
    ctx.textAlign = 'left';
    UI.shopActRects.push(r);
  };
  ...
  if (G.shopSilver && canExch) {
    ...
    if (e.item) {
      ...
      const it = e.item;
      const cur = counterpart(it);
      const cmp = [];
      if (cur && it.slot === 'weapon') { const d = Math.round(it.dmg / it.cd - cur.dmg / cur.cd); if (d) cmp.push({ s: (d > 0 ? '+' : '') + d + ' damage/sec vs equipped', c: d > 0 ? '#7fd0a0' : '#ff6b7a' }); }
      if (cur && it.slot === 'armor') { const d = it.armor - cur.armor; if (d) cmp.push({ s: (d > 0 ? '+' : '') + d + ' armor vs equipped', c: d > 0 ? '#7fd0a0' : '#ff6b7a' }); }
      if (cur) half(cur, zTop, topH, 'EQUIPPED');
      half(it, zMid + 2 * S, botH, cur ? 'NEW' : null, cmp);
    } else if (e.potion) {
      // отвара за отключване: прост инфо панел в долната половина
      const pd = POTIONS[e.potion];
      panel(zx, zMid + 2 * S, zw, botH);
      ctx.font = fontBold(8); ctx.fillStyle = pd.col;
      ctx.fillText(pd.n + ' (unlock)', zx + 8 * S, zMid + 16 * S);
      ctx.font = fontPx(6.5); ctx.fillStyle = '#a8b2c4';
    ...
    const sit = G.selItem;
    if (isMystic) {
      // Мистик: предметът ГОРЕ, афикс редовете ДОЛУ, Enchant най-долу
      half(sit, zTop, topH, null);
      let ay2 = zMid + 2 * S;
      const rh = 15 * S;
      ctx.textAlign = 'left';
      sit.affixes.forEach((a, ai) => {
        const d = AFFIXES[a.k]; if (!d) return;
        if (ay2 + rh > btnY - 4 * S) return; // в рамката
        const maxed = (a.up || 0) >= 3, cost = enchantCost(a);
        const hov = G.mouse.x >= zx && G.mouse.x < zx + zw && G.mouse.y >= ay2 && G.mouse.y < ay2 + rh;
        rcx(zx, ay2, zw, rh, (hov && !maxed) ? 'rgba(40,90,80,0.5)' : 'rgba(20,35,32,0.6)');
        if (hov && !maxed) strokeRect(zx, ay2, zw, rh, '#57e6c8', 1);
        ctx.font = fontPx(6); ctx.fillStyle = '#e8e4d0';
        const stars = '★'.repeat(a.up || 0) + '☆'.repeat(3 - (a.up || 0));
        ctx.fillText('+' + a.v + ' ' + d.n + ' ' + stars, zx + 4 * S, ay2 + 6.5 * S);
        ctx.font = fontPx(5.5); ctx.fillStyle = maxed ? '#7fd0a0' : ((G.meta.shards || 0) >= cost ? '#57e6c8' : '#ff6b7a');
        ctx.fillText(maxed ? 'fully upgraded' : '→ ' + Math.max(a.v + 1, Math.round(a.v * 1.15)) + '   (' + cost + ' shd)', zx + 4 * S, ay2 + 13 * S);
        if (!maxed) UI.enchAffixRects.push({ x: zx, y: ay2, w: zw, h: rh, ai });
        ay2 += rh + 2 * S;
      });
      const canScr = (G.meta.shards || 0) >= 10;
      actBtn('Enchant — reroll (10 shd)', midX - 60 * S, 120 * S, 'scramble', canScr ? '#c084ff' : '#5a677f');
    } else {
      // мой предмет: носеният срещу него ГОРЕ, избраният ДОЛУ; Продай/Ниво най-долу
  ...
  if (G.shopConfirm) {
    ...
    const mw = 190 * S, mh = 88 * S, mx0 = x0 + (pw - mw) / 2, my0 = y0 + (ph - mh) / 2;
    panel(mx0, my0, mw, mh);
    ...
    } else if (c.kind === 'unlock' && c.unlock) {
      title = 'Unlock'; body = c.unlock.t + (c.unlock.cost > 0 ? ' — for ' + c.unlock.cost + ' Abyss Seal' + (c.unlock.cost > 1 ? 's' : '') + '?' : ' — free. Take it?'); yesTxt = 'Take'; yesCol = '#c84fff';
    } else {
      title = 'Enchanting (irreversible)'; body = 'All affixes will be rerolled and upgrades reset. Are you sure?'; yesTxt = 'Enchant'; yesCol = '#c084ff';
    }
    ctx.textAlign = 'center'; ctx.font = fontBold(7.5); ctx.fillStyle = '#e8e4d0';
    ctx.fillText(title, mx0 + mw / 2, my0 + 14 * S);
    ctx.font = fontPx(6); ctx.fillStyle = '#a8b2c4';
    wrapTextCentered(body, mx0 + mw / 2, my0 + 27 * S, mw - 16 * S, 9 * S);
    const bw = 66 * S, bh = 16 * S, by = my0 + mh - 22 * S;
    const yb = { x: mx0 + mw / 2 - bw - 5 * S, y: by, w: bw, h: bh };
    const nb = { x: mx0 + mw / 2 + 5 * S, y: by, w: bw, h: bh };
    panel(yb.x, yb.y, yb.w, yb.h); ctx.font = fontBold(7); ctx.fillStyle = yesCol; ctx.fillText(yesTxt, yb.x + yb.w / 2, yb.y + 11 * S);
...
function shopClick(mx, my) {
  ...
  // действия върху избрания предмет — покупката и ъпгрейдите минават през ПОТВЪРЖДЕНИЕ
  ...
  // стока (ляво): тап МАРКИРА; сергията и отключванията — с потвърждение
  ...
  for (const r of UI.enchPotRects || []) if (hit(r)) { G.shopConfirm = { kind: 'potion', key: r.key }; return; } // ъпгрейд на отвара — с потвърждение
...
function menuFocusRects() {
  ...
  if (G.state === 'shop') {
    ...
    if (G.shopConfirm) return [...(UI.shopConfirmRects || [])]; // модалът е с приоритет за контролера
...
const CTRL_DEFS = {
  ...
  act:  { fx: 0.638, fy: 0.845, fr: 0.06,  name: 'Interact' }, // кръгъл, показва се само до обект; мести се като другите
};
function ctrlLayout() {
  const out = {};
...
function ctrlHit(px, py, includeAll) {
  ...
  for (const id in L) {
    if (!includeAll) {
      ...
      if (id === 'act' && !G.interactHint) continue; // скрит, когато няма обект наблизо
    }
    const c = L[id];
...
function drawTouchControls(editor) {
  ...
  drawPot(L.hp, 0);
  drawPot(L.mp, 1);
  // бутон за ВЗАИМОДЕЙСТВИЕ (кръгъл, местим) — вижда се само когато си до обект (огън/сергия/портал…)
  if (editor || G.interactHint) {
    const c = L.act;
    const pl = 0.45 + 0.2 * Math.sin(G.time * 5);
    circleBtn(c, 'rgba(255,210,59,0.16)', 'rgba(255,210,59,' + (editor ? '0.5' : pl.toFixed(2)) + ')');
    ctx.fillStyle = '#ffd23b';
    ctx.font = fontBold(11);
    ctx.textAlign = 'center';
    ctx.fillText('!', c.x, c.y + 4 * S);
    if (!editor && G.interactHint) { // какво ще направи — малък надпис над бутона
      ctx.font = fontPx(5.5);
      ctx.fillText(G.interactHint.txt.replace(/^E — /, ''), c.x, c.y - c.r - 4 * S);
    }
  }
  ctx.textAlign = 'left';
}
