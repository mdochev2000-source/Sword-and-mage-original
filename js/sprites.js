...
function genIntDoor() {
  ...
  for (let x = 0; x < 32; x++) {                                   // мазилка и греди
    ...
    for (let k = 0; k < H; k++) {
      ...
      if (x % 8 === 0 || k === 3) col = left ? beamL : beamR;
      px(g, x, yb + k, col);
    }
  }
...
function genInnkeeper() {
  // ГОСТИЛНИЧАРЯТ — по референцията на дизайнера, но в X век и в мащаба на
  // героя (22x26): ленена риза с навити ръкави, КОЖЕНА ПРЕСТИЛКА с презрамки
  // (вместо тиранти), зелен пояс, торбести гащи, навои и обувки.
  const g = mk(22, 26);
  const SKIN = '#d9a878', SKD = '#b3855a';
  const HAIR = '#2f2a25', HAIRL = '#463f36';                    // тъмна къдрава коса
  const LIN = '#e0dac6', LIND = '#b5ae99', LINS = '#8f8974';    // ленена риза
  const LTH = '#7d5a30', LTHD = '#5a3f20', LTHL = '#9a7440';    // кожена престилка
  const SASH = '#5a6b52', SASHD = '#44523e';                    // зелен пояс
  const TRS = '#5f6b57', TRSD = '#4a5444';                      // торбести гащи
  const WRAP = '#8a8570', BOOT = '#5c4022', BOOTD = '#3f2c16';  // навои и обувки

  // --- крака: торбести гащи, навои, обувки ---
  rc(g, 6, 19, 4, 4, TRS); rc(g, 12, 19, 4, 4, TRS);
  px(g, 6, 19, TRSD); px(g, 15, 19, TRSD);
  rc(g, 7, 22, 3, 2, WRAP); rc(g, 12, 22, 3, 2, WRAP);          // навои
  px(g, 8, 23, '#6e6a58'); px(g, 13, 23, '#6e6a58');
  rc(g, 6, 24, 4, 2, BOOT); rc(g, 12, 24, 4, 2, BOOT);
  rc(g, 6, 25, 4, 1, BOOTD); rc(g, 12, 25, 4, 1, BOOTD);

  // --- ленената риза с навити ръкави ---
  rc(g, 5, 13, 12, 7, LIN);
  rc(g, 5, 13, 4, 7, LIN); rc(g, 13, 13, 4, 7, LIND);           // сянка отдясно
  rc(g, 3, 14, 2, 4, LIN); rc(g, 17, 14, 2, 4, LIND);           // ръкави
  px(g, 3, 14, LIND); px(g, 18, 17, LINS);
  rc(g, 3, 18, 2, 2, SKIN); rc(g, 17, 18, 2, 2, SKIN);          // голи предмишници
  px(g, 3, 19, SKD); px(g, 18, 19, SKD);
  rc(g, 10, 13, 1, 5, LINS);                                    // разтворената яка

  // --- кожената престилка с презрамки (вместо тирантите от рисунката) ---
  rc(g, 6, 14, 2, 4, LTH); rc(g, 14, 14, 2, 4, LTH);            // презрамките
  px(g, 6, 14, LTHL); px(g, 15, 14, LTHD);
  rc(g, 7, 17, 8, 6, LTH);                                      // нагръдник и пола
  rc(g, 7, 17, 3, 6, LTHL);                                     // светлата страна
  rc(g, 13, 17, 2, 6, LTHD);
  px(g, 9, 19, LTHD); px(g, 12, 21, LTHD);                      // гънки/петна

  // --- зеленият пояс над престилката ---
  rc(g, 5, 18, 12, 1, SASH); rc(g, 5, 19, 12, 1, SASHD);
  px(g, 15, 18, '#6c8060');
  rc(g, 15, 19, 2, 3, SASH); px(g, 16, 21, SASHD);              // висящият край

  // --- лицето ---
  rc(g, 7, 5, 8, 7, SKIN);
  rc(g, 7, 5, 8, 1, SKD);
  px(g, 9, 7, '#1a1d26'); px(g, 12, 7, '#1a1d26');              // очи
  px(g, 8, 7, SKD); px(g, 13, 7, SKD);                          // вежди
  px(g, 10, 8, SKD); px(g, 11, 9, SKD);                         // нос
  rc(g, 9, 9, 4, 1, '#8a5b4a');                                 // усмивка
  rc(g, 8, 10, 6, 3, HAIR);                                     // брада
  px(g, 7, 10, HAIR); px(g, 14, 10, HAIR);
  rc(g, 9, 10, 2, 1, HAIRL); px(g, 12, 11, HAIRL);

  // --- КЪДРАВАТА коса: буйна, до врата ---
  rc(g, 7, 2, 8, 3, HAIR);
  rc(g, 6, 3, 1, 6, HAIR); rc(g, 15, 3, 1, 6, HAIR);
  px(g, 6, 2, HAIR); px(g, 15, 2, HAIR);                        // рошави краища
  px(g, 5, 5, HAIR); px(g, 16, 5, HAIR);
  px(g, 6, 9, HAIR); px(g, 15, 9, HAIR);
  rc(g, 8, 2, 3, 1, HAIRL); px(g, 7, 3, HAIRL);                 // блик
  px(g, 12, 2, '#241f1b'); px(g, 15, 6, '#241f1b');
  px(g, 8, 4, HAIR); px(g, 13, 4, HAIR);                        // кичури над челото

  // --- дървено канче в ръката ---
  rc(g, 19, 17, 3, 3, '#8a6a3a'); px(g, 19, 17, '#a3823f');
  px(g, 18, 18, '#8a6a3a');                                     // дръжката

  outlineSprite(g, '#10141c');
  return g.canvas;
}
// --- МЕБЕЛИ: изометрични обеми (плот с дебелина + крака, светла и тъмна страна) ---
// isoBox рисува кутия: ромбичен плот 2:1 + две странични лица надолу.
function isoBox(g, cx, cy, hw, hh, hgt, cTop, cL, cR, cEdge, cTopL) {
  for (let dx = -hw; dx <= hw; dx++) {
    const ext = Math.round(hh * (1 - Math.abs(dx) / (hw + 0.999)));
    for (let dy = -ext; dy <= ext; dy++) px(g, cx + dx, cy + dy, cTop);
    if (cTopL) px(g, cx + dx, cy - ext, cTopL);            // светъл ръб горе
    const yb = cy + ext;
    if (cEdge) px(g, cx + dx, yb, cEdge);                  // тъмен ръб долу
    for (let k = 1; k <= hgt; k++) px(g, cx + dx, yb + k, dx < 0 ? cL : cR);
  }
}
function isoLeg(g, x, y, h, c1, c2) { for (let k = 0; k < h; k++) { px(g, x, y + k, c1); px(g, x + 1, y + k, c2); } }
function woodGrain(g, R, x0, y0, w, h, cols) {
  for (let i = 0; i < w * h / 6; i++) {
    const x = x0 + ((R() * w) | 0), y = y0 + ((R() * h) | 0);
    if (g.getImageData(x, y, 1, 1).data[3]) px(g, x, y, cols[(R() * cols.length) | 0]);
  }
}
const WD = { top: '#96763f', topL: '#ab8b4f', left: '#6b5231', right: '#54401f', edge: '#3f2e1a', leg: '#5c4423', legD: '#3f2e1a' };

function genTable(R) {
  const g = mk(32, 26);
  isoLeg(g, 15, 6, 14, '#4a3620', '#33240f');                  // северният крак (зад плота)
  isoBox(g, 16, 9, 14, 7, 3, WD.top, WD.left, WD.right, WD.edge, WD.topL);
  woodGrain(g, R, 4, 4, 24, 10, ['#8a6b39', '#a1804a', '#7d5f33']);
  for (let dx = -9; dx <= 9; dx += 9) {                        // две фуги между дъските
    const ext = Math.round(7 * (1 - Math.abs(dx) / 14.999));
    for (let dy = -ext; dy <= ext; dy++) px(g, 16 + dx, 9 + dy, '#7a5c30');
  }
  isoLeg(g, 2, 13, 9, WD.leg, WD.legD);                        // западният
  isoLeg(g, 28, 13, 9, WD.leg, WD.legD);                       // източният
  isoLeg(g, 15, 17, 8, WD.leg, WD.legD);                       // южният
  rc(g, 4, 19, 24, 1, '#4a3620'); px(g, 3, 19, '#3f2e1a');     // напречна летва
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genBench(R) {
  const g = mk(28, 16);
  isoLeg(g, 13, 4, 8, '#4a3620', '#33240f');                   // северният крак
  isoBox(g, 14, 6, 12, 6, 2, '#8a6b39', '#63502f', '#4d3b1c', '#3a2b16', '#9c7c45');
  woodGrain(g, R, 4, 2, 20, 8, ['#7d5f33', '#93733e']);
  isoLeg(g, 2, 9, 5, WD.leg, WD.legD);
  isoLeg(g, 24, 9, 5, WD.leg, WD.legD);
  isoLeg(g, 13, 12, 3, WD.leg, WD.legD);
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genStool(R) {
  const g = mk(16, 16);
  isoBox(g, 8, 5, 6, 3, 2, '#8a6b39', '#63502f', '#4d3b1c', '#3a2b16', '#9c7c45');
  woodGrain(g, R, 4, 3, 8, 4, ['#7d5f33']);
  isoLeg(g, 2, 8, 6, WD.leg, WD.legD);
  isoLeg(g, 12, 8, 6, WD.leg, WD.legD);
  isoLeg(g, 7, 10, 5, '#4a3620', '#33240f');
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genKeg(R) {
  const g = mk(20, 24);
  // легнала бъчва на дървена стойка (както са в кръчмите)
  rc(g, 2, 17, 5, 4, '#4a3620'); rc(g, 2, 17, 5, 1, '#5c4423');   // стойка (клинове)
  rc(g, 13, 17, 5, 4, '#3f2e1a'); rc(g, 13, 17, 5, 1, '#54401f');
  for (let x = 2; x <= 17; x++) {                              // корем
    const t = Math.abs(x - 9.5) / 8.5;
    const half = Math.round(6 * (1 - t * t * 0.55));
    for (let y = 10 - half; y <= 10 + half; y++) {
      let col = y < 10 - half + 3 ? '#8a6a3a' : y > 10 + half - 3 ? '#54401f' : '#73562c';
      if (x === 5 || x === 6 || x === 13 || x === 14) col = (x === 5 || x === 13) ? '#4d525c' : '#33373d'; // железни обръчи
      px(g, x, y, col);
    }
  }
  for (let y = 5; y <= 15; y++) px(g, 2, y, '#4a3620');        // дъното
  for (let y = 6; y <= 14; y++) px(g, 17, y, '#96763f');       // капакът
  px(g, 16, 8, '#a3823f'); px(g, 16, 12, '#6b5231');
  woodGrain(g, R, 3, 6, 14, 8, ['#7d5f33', '#63502f']);
  px(g, 18, 13, '#241f1a'); px(g, 18, 14, '#241f1a');          // канелка
  px(g, 17, 16, '#c6a15a');                                    // капка
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genFireplace(R) {
  const g = mk(34, 32);
  const st = '#6d7280', stD = '#4a4e56', stL = '#8a9099', stX = '#5b6069', mort = '#3a3e46';
  const soot = '#2a2724';
  // ЗИДАНОТО тяло: широка основа, комин, стеснен нагоре
  for (let y = 0; y < 8; y++) rc(g, 8 + Math.round(y * 0.2), y, 18 - Math.round(y * 0.4), 1, y < 2 ? stL : st);
  rc(g, 1, 8, 32, 22, st);
  // камъни с фуги (редуващи се редове)
  for (let y = 9; y < 30; y += 4) {
    rc(g, 1, y, 32, 1, mort);
    for (let x = 2 + ((y % 8) ? 0 : 5); x < 33; x += 10) for (let k = 1; k < 4; k++) px(g, x, y + k, mort);
  }
  for (let i = 0; i < 90; i++) { const x = 1 + ((R() * 32) | 0), y = 8 + ((R() * 22) | 0); px(g, x, y, R() < 0.5 ? stX : stL); }
  // ДЪРВЕНАТА ГРЕДА-полица над огъня
  rc(g, 0, 13, 34, 3, '#6b5231'); rc(g, 0, 13, 34, 1, '#8a6a3a'); rc(g, 0, 15, 34, 1, '#3f2e1a');
  for (let x = 2; x < 33; x += 7) px(g, x, 14, '#54401f');
  // СВОДЪТ: дълбок отвор със заоблена арка и опушена задна стена
  for (let x = 6; x < 28; x++) {
    const t = Math.abs(x - 16.5) / 11;
    const top = 19 - Math.round(4 * (1 - t * t));
    for (let y = top; y < 29; y++) px(g, x, y, y < top + 3 ? soot : '#171310');
  }
  rc(g, 8, 24, 18, 5, '#241d17');                                  // задната стена в дъното
  for (let i = 0; i < 16; i++) { const x = 8 + ((R() * 18) | 0), y = 24 + ((R() * 5) | 0); px(g, x, y, '#332a20'); }
  // ВЕРИГА С КУКА (за котлето)
  for (let y = 16; y < 21; y++) px(g, 16, y, y % 2 ? '#5b6069' : '#8a9099');
  px(g, 16, 21, '#8a9099'); px(g, 17, 22, '#5b6069');
  // ковани стойки и дърва
  rc(g, 9, 27, 1, 3, '#3c4046'); rc(g, 24, 27, 1, 3, '#3c4046');
  rc(g, 9, 27, 16, 1, '#4d525c');
  rc(g, 11, 25, 12, 2, '#4a3620'); rc(g, 12, 24, 5, 1, '#5c4423'); rc(g, 18, 24, 4, 1, '#3f2e1a');
  px(g, 10, 26, '#2e2214'); px(g, 23, 26, '#2e2214');
  // ОГЪНЯТ: жар, три езика пламък, искри
  rc(g, 11, 22, 12, 3, '#c94f1f'); rc(g, 12, 21, 10, 1, '#e0651f');
  rc(g, 12, 19, 4, 3, '#ff8a1f'); rc(g, 17, 18, 4, 4, '#ff8a1f');
  rc(g, 13, 17, 2, 2, '#ffb84d'); rc(g, 18, 16, 2, 2, '#ffb84d');
  px(g, 14, 16, '#ffd23b'); px(g, 19, 15, '#ffd23b'); px(g, 16, 18, '#ffd23b');
  px(g, 15, 14, '#fff2a0'); px(g, 20, 14, '#fff2a0');
  px(g, 12, 24, '#ffd23b'); px(g, 21, 24, '#ff8a1f');              // жарава долу
  // опушено над отвора
  for (let x = 8; x < 26; x++) if (R() < 0.5) px(g, x, 16 + ((R() * 2) | 0), soot);
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genCauldron(R) {
  const g = mk(22, 24);
  // триножник
  for (let k = 0; k < 9; k++) { const o = Math.round(k * 0.35); rc(g, 3 - o, 13 + k, 2, 1, '#4d525c'); rc(g, 16 + o, 13 + k, 2, 1, '#33373d'); }
  for (let k = 0; k < 7; k++) rc(g, 10, 16 + k, 2, 1, '#3c4046');
  // тумбесто котле
  for (let y = 6; y <= 17; y++) {
    const t = (y - 6) / 11;
    const half = Math.round(8 * Math.sin(Math.PI * (0.28 + t * 0.62)));
    for (let x = 11 - half; x <= 10 + half; x++) {
      let col = x < 11 - half + 3 ? '#454b55' : x > 10 + half - 3 ? '#1e2126' : '#2b2f36';
      if (y === 6 || y === 7) col = x < 8 ? '#565d6e' : '#33373d';
      px(g, x, y, col);
    }
  }
  isoBox(g, 10, 7, 7, 3, 0, '#6b5424', '#4f3d18', '#3f3014', '#2e2410', '#7d6430'); // яхнията
  px(g, 8, 5, '#8a7048'); px(g, 12, 6, '#8a7048');              // мехури
  px(g, 10, 4, '#6b5d47'); px(g, 13, 4, '#5c4f3c');             // пара
  // дръжка
  for (let x = 2; x <= 18; x++) { const y = 3 - Math.round(2 * Math.sin(Math.PI * (x - 2) / 16)); px(g, x, y, '#3c4046'); }
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genShelf(R) {
  const g = mk(30, 30);
  const bd = '#7d5f33', bdL = '#96763f', bdD = '#5c4423';
  isoBox(g, 15, 13, 13, 6, 14, bd, '#63502f', '#4d3b1c', '#3f2e1a', bdL);   // тялото на шкафа
  // ВРАТИЧКИТЕ: рамка по лявото лице, следваща наклона
  for (let dx = -12; dx <= -1; dx++) {
    const ext = Math.round(6 * (1 - Math.abs(dx) / 13.999));
    const yTop = 13 + ext + 2, x = 15 + dx;
    for (let k = 0; k < 10; k++) px(g, x, yTop + k, k < 1 || k > 8 ? bdD : '#6e5530');
    if (dx === -12 || dx === -1 || dx === -7) for (let k = 0; k < 10; k++) px(g, x, yTop + k, bdD); // рамка и среда
  }
  px(g, 8, 22, '#3c4046'); px(g, 8, 26, '#3c4046');                          // панти
  px(g, 6, 24, '#c9a23b');                                                    // дръжка
  // делви и кани ОТГОРЕ
  const jar = (x, y, c1, c2, c3, h) => {
    rc(g, x, y + 1, 4, h, c1); rc(g, x, y + 1, 2, h, c2);
    rc(g, x + 1, y, 2, 1, c3); px(g, x + 4, y + 2, c3);
  };
  jar(4, 4, '#8a7a5e', '#a3947a', '#6b5d47', 5);
  jar(11, 2, '#6b5d47', '#7d6f57', '#54493a', 6);
  jar(18, 3, '#8a97ad', '#c6d3e6', '#67738c', 5);
  jar(24, 5, '#7d6b52', '#94816a', '#5c4f3c', 4);
  woodGrain(g, R, 3, 10, 24, 16, ['#6e5530', '#87683a']);
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genBedRoll(R) {
  const g = mk(32, 20);
  // рамка от дъски + сламеник + завивка
  isoBox(g, 16, 12, 15, 7, 2, '#5c4423', '#47341a', '#382812', '#241d12', '#6b5231');
  isoBox(g, 16, 10, 13, 6, 1, '#c9a463', '#a8863f', '#8f6f34', '#6b5231', '#dcc084'); // слама
  woodGrain(g, R, 5, 5, 22, 9, ['#b08a48', '#dcc084', '#c09a56']);
  isoBox(g, 9, 8, 6, 3, 2, '#e0dac6', '#bab399', '#9c957c', '#7d7660', '#f0ebd9');   // възглавница
  isoBox(g, 21, 11, 8, 4, 2, '#5e4a6b', '#463553', '#372943', '#241a2c', '#75608a'); // завивка
  px(g, 26, 12, '#8a74a3'); px(g, 18, 13, '#453552');
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genSacks(R) {
  const g = mk(26, 24);
  // профилът е зададен ред по ред: 2-3 реда врат, издути рамене, широко тяло
  const sack = (cx, base, prof, c1, cL, cD, rope) => {
    for (let k = 0; k < prof.length; k++) {
      const half = prof[k], y = base - prof.length + k;
      for (let x = cx - half; x <= cx + half; x++) {
        px(g, x, y, x <= cx - half + 2 ? cL : (x >= cx + half - 1 ? cD : c1));
      }
    }
    const nh = prof[0];
    rc(g, cx - nh, base - prof.length - 1, nh * 2 + 1, 1, rope);          // въжето
    px(g, cx - nh - 1, base - prof.length, cD); px(g, cx + nh + 1, base - prof.length, cD);
    rc(g, cx - 1, base - prof.length - 3, 3, 2, c1);                      // гърлото над възела
    px(g, cx - 2, base - prof.length - 2, cL); px(g, cx + 2, base - prof.length - 2, cD);
  };
  sack(7, 22, [2, 3, 4, 5, 6, 6, 7, 7, 7, 7, 7, 6, 6], '#a3947a', '#c0b295', '#7d6f57', '#6b5d47');
  sack(18, 23, [2, 3, 4, 5, 5, 6, 6, 6, 6, 5], '#8a7a5e', '#a3947a', '#665a45', '#54493a');
  for (let i = 0; i < 34; i++) { const x = 1 + ((R() * 24) | 0), y = 8 + ((R() * 15) | 0); if (g.getImageData(x, y, 1, 1).data[3]) px(g, x, y, R() < 0.5 ? '#94866c' : '#77694f'); }
  px(g, 4, 15, '#7d6f57'); px(g, 5, 18, '#7d6f57'); px(g, 9, 17, '#8f8168'); // гънки
  px(g, 16, 19, '#665a45'); px(g, 20, 18, '#77694f');
  outlineSprite(g, '#10131c');
  return g.canvas;
}
...
// след построяване на повърхностните спрайтове се прилагат пикселните корекции на дизайнера
...
function genPeddler() {
  // СТРАНСТВАЩИЯТ ТЪРГОВЕЦ — X век: плъстена шапка с тесен кант, винено-сливово
  // наметало, тъмночервен шал, СРЕБЪРНИ везни на пояса (знакът на търговеца),
  // кош на гръб, тояга и навои. Чиби пропорцията и контурът са като на героя.
  const g = mk(22, 26);
  const BAND = '#6b4a2a';                                       // кожен кант
  const SKIN = '#d9a878', SKD = '#b3855a';
  const BRD = '#2b2730', BRDL = '#3d3844';                      // черна брада
  const SCF = '#8f2233', SCFL = '#b13a4b';                      // шал
  const CLK = '#48294a', CLKL = '#5e3a60', CLKD = '#301b32';    // наметало
  const LIN = '#d8d2be', LIND = '#aca691';                      // ленена риза
  const LTH = '#6b4a2a', LTHD = '#4a3320';                      // кожа
  const SIL = '#c6c9d1', SILD = '#8f96a3';                      // сребро
  const WCK = '#8a6a3a', WCKD = '#63492a';                      // плетен кош
  const DKW = '#3a2f24';                                        // навои/обувки

  // --- кошът на гръб (изнича зад дясното рамо) ---
  rc(g, 16, 10, 4, 9, WCK);
  rc(g, 16, 10, 4, 1, WCKD);
  for (let y = 12; y < 19; y += 2) rc(g, 16, y, 4, 1, WCKD);    // плетка
  px(g, 19, 13, WCKD); px(g, 19, 17, WCKD);
  rc(g, 16, 8, 3, 2, LTH); px(g, 16, 7, LTHD);                  // вързоп отгоре

  // --- крака: ленени гащи + навои ---
  rc(g, 8, 21, 3, 3, LIN); rc(g, 12, 21, 3, 3, LIN);
  px(g, 8, 21, LIND); px(g, 12, 21, LIND);
  rc(g, 8, 22, 3, 1, DKW); rc(g, 12, 23, 3, 1, DKW);            // кръстосани навои
  px(g, 9, 23, DKW); px(g, 13, 22, DKW);
  rc(g, 8, 24, 3, 2, DKW); rc(g, 12, 24, 3, 2, DKW);            // обувки от една кожа
  rc(g, 8, 25, 3, 1, '#241d16'); rc(g, 12, 25, 3, 1, '#241d16');

  // --- наметалото (тяло) ---
  rc(g, 4, 14, 14, 8, CLK);
  rc(g, 5, 22, 12, 1, CLK); rc(g, 6, 23, 10, 1, CLKD);          // подгъв
  rc(g, 4, 14, 5, 8, CLKL);                                     // осветената лява пола
  rc(g, 9, 15, 1, 8, CLKD); rc(g, 14, 16, 1, 6, CLKD);          // гънки
  px(g, 4, 21, CLKD); px(g, 17, 21, CLKD);

  // --- ленените ръкави ---
  rc(g, 3, 15, 2, 5, LIN); px(g, 3, 15, LIND); px(g, 3, 19, LIND);
  rc(g, 17, 15, 2, 5, LIN); px(g, 18, 19, LIND);
  px(g, 3, 20, SKIN); px(g, 4, 20, SKIN);                       // длани
  px(g, 17, 20, SKIN); px(g, 18, 20, SKIN);

  // --- поясът: кесия и СРЕБЪРНИТЕ ВЕЗНИ (знакът на търговеца) ---
  rc(g, 4, 19, 14, 1, LTH); rc(g, 4, 20, 14, 1, LTHD);
  rc(g, 10, 19, 2, 1, SIL);                                     // катарама
  rc(g, 14, 20, 3, 3, LTH); px(g, 14, 20, LTHD); px(g, 16, 22, LTHD); // кесия
  px(g, 6, 20, SILD);                                           // ВЕЗНИТЕ на хълбока
  rc(g, 5, 21, 3, 1, SIL);                                      // рамото на кантара
  px(g, 5, 22, SILD); px(g, 7, 22, SILD);                       // двете блюда

  // --- шалът около врата ---
  rc(g, 6, 13, 10, 1, SCF);
  rc(g, 7, 13, 4, 1, SCFL);
  px(g, 5, 13, SCF); px(g, 16, 13, SCF);
  rc(g, 15, 14, 2, 4, SCF); px(g, 16, 17, SCFL);                // край, паднал отпред
  // ленената риза наднича под шала
  rc(g, 10, 14, 2, 1, LIN); px(g, 10, 15, LIND);
  // сребърният медальон на верижка
  px(g, 10, 16, SIL); px(g, 11, 16, SIL); px(g, 10, 17, SILD); px(g, 11, 17, SIL);
  px(g, 9, 15, SILD); px(g, 12, 15, SILD);

  // --- лицето (под периферията) ---
  rc(g, 7, 5, 8, 7, SKIN);
  px(g, 7, 5, SKD); px(g, 14, 5, SKD);                          // скули
  px(g, 9, 7, '#1a1d26'); px(g, 12, 7, '#1a1d26');              // очи
  px(g, 8, 7, SKD); px(g, 13, 7, SKD);                          // вежди/сенки
  px(g, 10, 8, SKD); px(g, 11, 9, SKD);                         // нос
  rc(g, 7, 10, 8, 3, BRD);                                      // ШИРОКА брада
  px(g, 7, 8, BRD); px(g, 14, 8, BRD);                          // бакенбарди
  px(g, 7, 9, BRD); px(g, 14, 9, BRD);
  rc(g, 8, 10, 2, 1, BRDL); px(g, 13, 11, BRDL);                // блик
  rc(g, 9, 9, 4, 1, '#8a5b4a');                                 // устна линия

  // --- ПЛЕШИВО ТЕМЕ (косата е махната): гола глава с блик и загар ---
  rc(g, 8, 1, 6, 1, SKIN);                                       // темето
  rc(g, 7, 2, 8, 3, SKIN);
  rc(g, 9, 1, 3, 1, '#eec394'); rc(g, 8, 2, 3, 1, '#eec394');    // блик горе-ляво
  px(g, 14, 2, SKD); px(g, 14, 3, SKD); px(g, 7, 4, SKD);        // заобляне
  px(g, 8, 4, SKD); px(g, 13, 4, SKD);                           // сянка на слепоочията
...
function genRock3(R) {
  // голям валун
  const g = mk(20, 15);
  rc(g, 3, 5, 13, 8, '#565a62'); rc(g, 5, 3, 9, 3, '#5c6068'); rc(g, 7, 2, 5, 1, '#5c6068');
  rc(g, 15, 8, 4, 5, '#4a4e56');
  rc(g, 3, 5, 1, 7, '#6d7280'); rc(g, 5, 3, 1, 3, '#6d7280'); px(g, 7, 2, '#767b85'); // светъл ръб
  rc(g, 13, 4, 2, 8, '#41454d'); rc(g, 9, 9, 4, 3, '#4a4e56');                         // сенки
  px(g, 6, 7, '#63676f'); px(g, 10, 5, '#63676f'); px(g, 8, 11, '#3c4046');
  px(g, 4, 12, '#3c4046'); px(g, 16, 12, '#33373d');
  px(g, 4, 13, '#4f6a45'); px(g, 14, 13, '#4f6a45'); px(g, 17, 11, '#4f6a45');         // мъх
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genFence2(R) {
  // ПОВРЕДЕНА ограда: наклонени и липсващи дъски
  const g = mk(16, 12);
  rc(g, 2, 4, 2, 8, '#4a3a28'); px(g, 2, 4, '#5c4a34');            // ляв кол (здрав)
  rc(g, 12, 6, 2, 6, '#4a3a28'); px(g, 12, 6, '#3a2d1e');          // десен кол (изкривен, по-нисък)
  px(g, 13, 5, '#4a3a28');
  // счупена горна летва — виси надолу
  px(g, 4, 6, '#5c4a34'); px(g, 5, 7, '#5c4a34'); px(g, 6, 8, '#4a3a28'); px(g, 7, 9, '#4a3a28');
  // долната летва е наполовина
  rc(g, 4, 9, 5, 1, '#5c4a34'); px(g, 9, 10, '#3a2d1e');
  px(g, 3, 11, '#33291f'); px(g, 10, 11, '#33291f');               // паднали трески
  outlineSprite(g, '#10131c');
  return g.canvas;
}
function genFence3(R) {
  // ОБГОРЕНА ограда: почерняла частично, овъглени върхове
  const g = mk(16, 12);
  rc(g, 2, 3, 2, 9, '#2b241c'); px(g, 2, 3, '#1a1512'); px(g, 3, 4, '#3a2d1e');   // овъглен кол
  rc(g, 12, 4, 2, 8, '#4a3a28'); px(g, 12, 4, '#2b241c'); px(g, 13, 3, '#1a1512'); // полуобгорен кол
  rc(g, 4, 6, 8, 1, '#3a2d1e'); px(g, 5, 6, '#2b241c'); px(g, 8, 6, '#1a1512');   // летва с изгорели петна
  rc(g, 4, 9, 6, 1, '#2b241c'); px(g, 9, 9, '#1a1512');
  px(g, 6, 5, '#57320f'); px(g, 11, 5, '#6a4a1a');                                 // тлеещи ръбове
  px(g, 4, 11, '#1a1512'); px(g, 8, 11, '#1a1512');                                // пепел
  outlineSprite(g, '#10131c');
  return g.canvas;
}
...
function genDeadTree2(R) {
  // чворест мъртъв дъб: къс дебел ствол, широко разперени криви клони (канделабър)
  const g = mk(26, 24);
  const d1 = '#3a3126', d2 = '#4a4033', d3 = '#2e2820', hl = '#54493a';
  rc(g, 11, 16, 4, 7, d1); rc(g, 11, 16, 1, 7, d2); px(g, 11, 16, hl);   // дебел ствол
  px(g, 9, 22, d3); px(g, 8, 23, d1); px(g, 16, 22, d3); px(g, 17, 23, d2); // корени
  rc(g, 11, 14, 4, 2, d2);                                                // разширение горе
  // ляв кандел: настрани и нагоре
  px(g, 10, 13, d1); px(g, 8, 12, d2); px(g, 6, 12, d1); px(g, 4, 11, d3);
  px(g, 4, 9, d2); px(g, 3, 7, d3);                                       // завой нагоре
  px(g, 7, 10, d2); px(g, 7, 8, d3); px(g, 8, 7, d1);                     // разклонка
  // десен кандел
  px(g, 15, 13, d1); px(g, 17, 12, d2); px(g, 19, 12, d1); px(g, 21, 11, d3);
  px(g, 21, 9, d2); px(g, 22, 7, d3);
  px(g, 18, 10, d2); px(g, 18, 8, d3); px(g, 17, 7, d1);
  // среден къс клон
  px(g, 12, 12, d2); px(g, 13, 10, d1); px(g, 12, 8, d3); px(g, 13, 6, d2);
  px(g, 14, 12, d3); px(g, 10, 18, d3); px(g, 13, 20, d3);                // чепове
  outlineSprite(g, '#10131c');
  return g.canvas;
}
...
// 2.5D гарнизонна кула-КИЙП върху 2x2 клетки: масивна, 64x156 (pyo=16)
function genMirTower(R) {
  const FH = 112, oy = 12; // корона 0..11, капак-ромб 12..43, лица 44..155
  const g = mk(64, oy + 32 + FH);
  const st = '#5c6068', stD = '#4a4e56', stL = '#6d7280', mort = '#3a3e46';
  // капакът (бойната площадка) — ромб 64x32
  for (let yy = 0; yy < 32; yy++) {
    const half = yy < 16 ? (yy + 1) * 2 : (32 - yy) * 2;
    rc(g, 32 - half, oy + yy, half * 2, 1, st);
  }
  for (let i = 0; i < 60; i++) {
    const x = 4 + ((R() * 56) | 0), yy = 2 + ((R() * 28) | 0);
    px(g, x, oy + yy, R() < 0.5 ? stD : stL);
  }
  // лицата: масивна зидария с фуги, ъглов ръб и тъмна основа
  for (let x = 0; x < 64; x++) {
    const sw = x < 32, ey = iso2EdgeY(oy, x);
    for (let k = 1; k <= FH; k++) {
      let col = sw ? '#4e525a' : '#41454d';
      if (k % 7 === 6) col = mort;
      else if (((x + ((k / 7) | 0) * 5) % 11) === 0) col = mort;
      if (k >= FH - 3) col = k >= FH - 1 ? '#20242b' : '#2b2f36';
      px(g, x, ey + k, col);
    }
    px(g, x, ey, sw ? stL : stD);
  }
  for (let k = 1; k <= FH; k++) { px(g, 31, iso2EdgeY(oy, 31) + k, stL); px(g, 32, iso2EdgeY(oy, 32) + k, '#383c44'); } // южният ъгъл
  // шум по камъка
  for (let i = 0; i < 60; i++) {
    const x = 1 + ((R() * 62) | 0), ey = iso2EdgeY(oy, x);
    const k = 2 + ((R() * (FH - 8)) | 0);
    px(g, x, ey + k, R() < 0.5 ? '#454952' : '#565a62');
  }
  // ПРОЗОРЦИ: бойници с рамка и топли прозорци, подредени по ЕТАЖИ в колони
  const slit = (x0, top) => {
    for (let x = x0; x < x0 + 3; x++) { const ey = iso2EdgeY(oy, x); for (let k = top; k < top + 10; k++) px(g, x, ey + k, '#12100c'); }
    const em = iso2EdgeY(oy, x0 + 1);
    rc(g, x0 - 1, em + top - 1, 5, 1, stD);            // трегер отгоре
    rc(g, x0 - 1, em + top + 10, 5, 1, stL);           // перваз отдолу
    px(g, x0 + 1, em + top + 2, '#242830');            // дълбочина
  };
  const warm = (x0, top) => {
    for (let x = x0; x < x0 + 6; x++) { const ey = iso2EdgeY(oy, x); for (let k = top; k < top + 7; k++) px(g, x, ey + k, '#171310'); }
    const em = iso2EdgeY(oy, x0 + 2);
    rc(g, x0 - 1, em + top - 1, 8, 1, stD);
    rc(g, x0 - 1, em + top + 7, 8, 1, stL);
    px(g, x0 + 1, em + top + 2, '#e8a84d'); px(g, x0 + 3, em + top + 3, '#c98a3b'); px(g, x0 + 4, em + top + 2, '#8a6420');
  };
  // фасада (СЗ): колона бойници на 3 етажа + топъл прозорец на 3-тия
  slit(8, 18); slit(8, 46); slit(8, 74);
  warm(16, 74);
  // страница (СИ): колона бойници на 2 етажа + топъл прозорец
  slit(46, 32); slit(46, 60);
  warm(52, 88);
  // ВРАТАТА: арка със стъпала на фасадата
  for (let x = 11; x < 22; x++) {
    const ey = iso2EdgeY(oy, x);
    for (let k = FH - 17; k <= FH - 4; k++) px(g, x, ey + k, k === FH - 17 ? '#241f1a' : '#1a140e');
  }
  { const ey = iso2EdgeY(oy, 12); rc(g, 12, ey + FH - 19, 9, 2, stD); px(g, 20, ey + FH - 9, '#9a9da5'); }
  // факла до вратата
  { const ey = iso2EdgeY(oy, 24); px(g, 24, ey + FH - 14, '#241f1a'); px(g, 24, ey + FH - 15, '#ffb84d'); px(g, 24, ey + FH - 16, '#c98a3b'); }
  // КОРОНАТА: зъбери, стъпили ТОЧНО на ръбовете на площадката, по целия периметър
  const merlon = (mx, edgeYv) => {
    rc(g, mx, edgeYv - 6, 4, 6, st);
    px(g, mx, edgeYv - 6, stL); rc(g, mx + 3, edgeYv - 5, 1, 5, stD);
    px(g, mx + 1, edgeYv - 1, stD);
  };
  // задните ръбове (СЗ и СИ на ромба): y = oy + 16 -/+ ...
  for (const mx of [2, 10, 18, 26]) merlon(mx, oy + 16 - (mx >> 1));        // заден ляв ръб
  merlon(30, oy + 1);                                                        // северният ъгъл
  for (const mx of [36, 44, 52]) merlon(mx, oy + ((mx + 4 - 32) >> 1) + 1); // заден десен ръб
  merlon(59, oy + 15);                                                       // източният ъгъл
  // предните ръбове (парапетът пред площадката)
  for (const mx of [4, 12, 20]) merlon(mx, iso2EdgeY(oy, mx + 2) + 1);      // преден ляв
  merlon(29, oy + 32);                                                       // южният ъгъл
  for (const mx of [38, 46, 54]) merlon(mx, iso2EdgeY(oy, mx + 2) + 1);     // преден десен
  merlon(0, oy + 16);                                                        // западният ъгъл
  outlineSprite(g, '#10131c');
  return g.canvas;
}
...
function initSurfaceSprites() {
  ...
  Spr.surf = {
    ...
    table: genTable(R), bench: genBench(R), stool: genStool(R), keg: genKeg(R),
    fireplace: genFireplace(R), cauldron: genCauldron(R), shelf: genShelf(R),
  ...
  if (typeof applySpriteOverrides === 'function') applySpriteOverrides(); // пикселните корекции на дизайнера
}
