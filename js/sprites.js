...
function iso2Body(g, R, oy, wallH, roofH, pal) {
  ...
  for (let x = 0; x < 32; x++) px(g, x + 16, iso2EdgeY(oy, x) - N, pal.rD); // билото
}
...
function genMirHouse(R, v, twoStory) {
  ...
  // комин: малък, ЗАД билото — вижда се само върхът, шевът е скрит от покрива
  rc(g, 44, 14, 4, 7, '#565a62');                                // тялото (до билото на y=20)
  rc(g, 44, 14, 1, 7, '#6d7280'); rc(g, 47, 14, 1, 7, '#4a4e56');
  px(g, 45, 16, '#3a3e46'); px(g, 46, 18, '#3a3e46');            // фуги
  rc(g, 43, 13, 6, 2, '#4a4e56'); rc(g, 43, 13, 6, 1, '#6d7280'); // капак-плоча
  rc(g, 45, 12, 2, 1, '#171310'); px(g, 44, 12, '#2b2f36');      // отворът и саждите
...
function genMirTower(R) {
  ...
  // фенерът: златен капак, ковано тяло, топъл пламък (мъждука в рендера)
  px(g, 15, 2, '#e8c04a'); rc(g, 13, 3, 6, 1, '#c9a23b');       // връх + златен капак
  rc(g, 12, 4, 8, 6, '#241f1a');                                 // тялото
  rc(g, 14, 5, 4, 4, '#ffcf6a'); px(g, 14, 5, '#fff2c0'); px(g, 17, 8, '#e8a84d'); // пламъкът
  px(g, 12, 6, '#382d20'); px(g, 19, 6, '#382d20');              // рамка
  rc(g, 15, 10, 2, 5, '#382d20');                                // столче към площадката
  outlineSprite(g, '#10131c');
  return g.canvas;
}
...
function genChurch(R) {
  ...
  // КАМБАНАРИЯТА: рисува се ПРЕДИ покрива -> той скрива основата ѝ (никакъв шев)
  rc(g, 34, 14, 12, 24, '#565a62');
  rc(g, 34, 14, 2, 24, '#6d7280'); rc(g, 44, 14, 2, 24, '#4a4e56');
  for (let y = 18; y < 36; y += 4) rc(g, 35, y, 10, 1, '#3a3e46');
  rc(g, 37, 20, 6, 7, '#12100c'); px(g, 39, 22, '#8a8d95'); px(g, 39, 23, '#6a6d75'); // камбаната
  px(g, 37, 19, '#4a4e56'); px(g, 42, 19, '#4a4e56');
  for (let i = 0; i < 5; i++) rc(g, 35 + i, 13 - i * 2, 10 - i * 2, 2, i % 2 ? '#232830' : '#2d333c'); // шпилът
  rc(g, 39, 0, 2, 8, '#c9a23b'); rc(g, 37, 2, 6, 2, '#c9a23b'); px(g, 39, 0, '#e8c04a');   // КРЪСТЪТ
  iso2Body(g, R, oy, wallH, roofH, pal);
