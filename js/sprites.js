...
function genMirHouse(R, v, twoStory) {
  ...
  // комин: каменен, слят със ската — основата следва наклона на покрива
  for (let cxx = 0; cxx < 4; cxx++) {
    const bot = 24 - (cxx >> 1);
    for (let y = 10; y <= bot; y++) {
      let col = cxx === 0 ? '#6d7280' : cxx === 3 ? '#4a4e56' : '#565a62';
      if (y % 4 === 1 && y > 10) col = '#3a3e46';
      px(g, 44 + cxx, y, col);
    }
  }
  rc(g, 43, 9, 6, 2, '#4a4e56'); rc(g, 43, 9, 6, 1, '#6d7280'); // капак-плоча
  rc(g, 45, 8, 2, 1, '#171310'); px(g, 44, 8, '#2b2f36');       // отворът и саждите
...
function genGateTower(R) {
  ...
  const FH = 46, oy = 14; // мерлони 0..13, диамант 14..29, лица 30..75 (+1 блок)
  // капак
  for (let y = 0; y < 16; y++) { const [x0, w2] = diamondSpan(y); rc(g, x0, oy + y, w2, 1, st); }
...
function genChurch(R) {
  ...
  // КАМБАНАРИЯТА: по-висока, стъпила ОТЗАД върху билото (основата потъва в ската)
  rc(g, 36, 12, 12, 26, '#565a62');
  rc(g, 36, 12, 2, 26, '#6d7280'); rc(g, 46, 12, 2, 26, '#4a4e56');
  for (let y = 16; y < 36; y += 4) rc(g, 37, y, 10, 1, '#3a3e46');
  rc(g, 39, 18, 6, 7, '#12100c'); px(g, 41, 20, '#8a8d95'); px(g, 41, 21, '#6a6d75'); // камбаната
  px(g, 39, 17, '#4a4e56'); px(g, 44, 17, '#4a4e56');
  for (let i = 0; i < 5; i++) rc(g, 37 + i, 11 - i * 2, 10 - i * 2, 2, i % 2 ? '#232830' : '#2d333c'); // шпилът
  rc(g, 41, 0, 2, 8, '#c9a23b'); rc(g, 39, 2, 6, 2, '#c9a23b'); px(g, 41, 0, '#e8c04a');   // КРЪСТЪТ (дълго долно рамо)
  outlineSprite(g, '#10131c');
  return g.canvas;
}
