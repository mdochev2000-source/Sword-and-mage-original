...
function genPeddler() {
  ...
  rc(g, 7, 5, 8, 7, SKIN);
  rc(g, 7, 5, 8, 1, SKD);                                       // сянка от косата
  px(g, 9, 7, '#1a1d26'); px(g, 12, 7, '#1a1d26');              // очи
  px(g, 8, 7, SKD); px(g, 13, 7, SKD);                          // вежди/сенки
  px(g, 10, 8, SKD); px(g, 11, 9, SKD);                         // нос
  rc(g, 8, 10, 6, 3, BRD);                                      // брада — по-тясна
  px(g, 7, 10, BRD); px(g, 14, 10, BRD);
  rc(g, 9, 10, 2, 1, BRDL); px(g, 12, 11, BRDL);                // блик по брадата
  rc(g, 9, 9, 4, 1, '#8a5b4a');                                 // устна линия

  // --- КОСАТА (в цвета на брадата): теме и кичури до скулите ---
  rc(g, 7, 2, 8, 3, BRD);                                        // теме
  rc(g, 6, 3, 1, 6, BRD); rc(g, 15, 3, 1, 6, BRD);               // кичури край лицето
  px(g, 6, 9, BRD); px(g, 15, 9, BRD);                           // краищата им
  rc(g, 8, 2, 3, 1, BRDL); px(g, 7, 3, BRDL); px(g, 8, 3, BRDL); // блик горе-ляво
  px(g, 13, 2, '#1e1b23'); px(g, 15, 5, '#1e1b23');              // сянка вдясно
  px(g, 7, 5, BRD); px(g, 14, 5, BRD);                           // кичури над челото
