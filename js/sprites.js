...
function genPeddler() {
  ...
  const g = mk(22, 26);
  const HAT = '#b08a48', HATD = '#8a6a34', HATL = '#c9a463';   // плъст (охра)
  ...
  const WOOD = '#6b5636', WOODL = '#8a7048';                    // тояга
  const DKW = '#3a2f24';                                        // навои/обувки

  // --- тоягата (зад всичко, отляво) ---
  rc(g, 1, 3, 1, 21, WOOD);
  px(g, 1, 3, WOODL); px(g, 1, 8, WOODL); px(g, 1, 16, WOODL);
  px(g, 2, 5, WOOD); px(g, 0, 12, WOOD);                        // чепове
  ...
  px(g, 3, 20, SKIN); px(g, 4, 20, SKIN);                       // длан на тоягата
  ...
  rc(g, 7, 6, 8, 6, SKIN);
  rc(g, 7, 6, 8, 2, SKD);                                       // сянка от периферията
  px(g, 9, 8, '#1a1d26'); px(g, 12, 8, '#1a1d26');              // очи
  px(g, 10, 9, SKD);                                            // нос
  rc(g, 7, 10, 8, 3, BRD);                                      // черна брада
  rc(g, 8, 10, 2, 1, BRDL); px(g, 13, 11, BRDL);
  px(g, 7, 9, BRD); px(g, 14, 9, BRD);                          // бакенбарди
  rc(g, 9, 10, 4, 1, '#5a3a34');                                // устна линия

  // --- плъстената шапка: ниска корона + ТЕСЕН кант ---
  rc(g, 8, 0, 6, 5, HAT);
  rc(g, 8, 0, 3, 1, HATL); px(g, 8, 1, HATL); px(g, 8, 2, HATL); // блик
  px(g, 13, 1, HATD); px(g, 13, 3, HATD);                        // заоблена корона
  rc(g, 8, 4, 6, 1, BAND); px(g, 12, 4, '#8f6a34');              // кожената лента
  rc(g, 6, 5, 10, 1, HAT);                                       // тесният кант
  rc(g, 7, 6, 8, 1, HATD);                                       // сянката му
  px(g, 6, 5, HATL); px(g, 15, 5, HATD);
