...
function genFireplace(R) {
  const g = mk(32, 30);
  const st = '#6d7280', stD = '#4a4e56', stL = '#878d9c', stX = '#585d68';
  // ЗИДАНИЯТ комин: стеснява се нагоре
  for (let y = 0; y < 10; y++) {
    const inset = 4 - Math.round(y * 0.35);
    rc(g, 4 + inset, y, 24 - inset * 2, 1, y < 2 ? stL : st);
  }
  rc(g, 2, 10, 28, 3, st); rc(g, 2, 10, 28, 1, stL);           // полицата над огъня
  rc(g, 2, 13, 28, 14, st);                                     // тялото
  // камъни с фуги
  for (let y = 14; y < 27; y += 3) {
    rc(g, 2, y, 28, 1, stD);
    for (let x = 3 + ((y % 6) ? 0 : 4); x < 30; x += 8) px(g, x, y + 1, stD), px(g, x, y + 2, stD);
  }
  for (let i = 0; i < 60; i++) { const x = 2 + ((R() * 28) | 0), y = 13 + ((R() * 14) | 0); px(g, x, y, R() < 0.5 ? stX : stL); }
  // ОТВОРЪТ: арка
  rc(g, 8, 17, 16, 10, '#120f0d');
  for (let x = 8; x < 24; x++) {                                // заоблена арка
    const t = Math.abs(x - 15.5) / 8;
    const top = 17 - Math.round(3 * (1 - t * t));
    for (let y = top; y < 17; y++) px(g, x, y, '#120f0d');
  }
  rc(g, 7, 26, 18, 2, stD); rc(g, 7, 27, 18, 1, '#3a3e46');    // огнището (под)
  // дърва и огън
  rc(g, 10, 23, 12, 2, '#4a3620'); rc(g, 11, 22, 4, 1, '#5c4423'); rc(g, 17, 22, 4, 1, '#3f2e1a');
  px(g, 10, 24, '#2e2214'); px(g, 21, 24, '#2e2214');
  rc(g, 11, 19, 10, 4, '#c94f1f');                              // жар
  rc(g, 12, 17, 8, 3, '#ff8a1f'); rc(g, 13, 15, 6, 3, '#ffb84d');
  rc(g, 14, 13, 4, 2, '#ffd23b'); px(g, 15, 12, '#fff2a0'); px(g, 17, 13, '#fff2a0');
  px(g, 12, 21, '#ffd23b'); px(g, 19, 20, '#ffd23b');
  outlineSprite(g, '#10131c');
  return g.canvas;
}
...
function genCandleStand() {
  const g = mk(12, 26);
  isoBox(g, 6, 21, 5, 2, 2, '#565a62', '#3c4046', '#2b2f36', '#20242b', '#6d7280'); // стъпало
  for (let y = 8; y <= 21; y++) { px(g, 5, y, '#6d7280'); px(g, 6, y, '#454b55'); }  // стълб
  px(g, 4, 13, '#565a62'); px(g, 7, 16, '#3c4046');                                  // възелчета
  isoBox(g, 6, 7, 4, 2, 1, '#6d7280', '#454b55', '#33373d', '#24272c', '#878d9c');   // чиния
  rc(g, 5, 2, 2, 5, '#efe9d6'); px(g, 5, 2, '#fdf8ec'); px(g, 6, 6, '#cdc6b0');      // свещ
  px(g, 5, 1, '#ffb84d'); px(g, 5, 0, '#ffd23b'); px(g, 6, 1, '#ff8a1f');            // пламъче
  px(g, 4, 4, '#d8d2be');                                                             // капнал восък
  outlineSprite(g, '#10131c');
  return g.canvas;
}
